...
const getDoctorDetails=async (req,res)=>{
    try{
        const doctors= await doctor.find().sort({ name: 1 }).select("-_id").select("-__v");
...
const createDoctor = async (req, res) => {
    try {
        ...
        if (!name || !doctor_id ||  !specialization) {
...
const getDoctorDetail = async (req, res) => {
    try {
        const doc = await doctor.findOne({ doctor_id: req.params.id },{_id:0,__v: 0});
...
const updateDoctor = async (req, res) => {
    try {
        const doc = await doctor.findOne({ doctor_id: req.params.id });
...
const deleteDoctor = async (req, res) => {
    try {
        ...
        await doctor.deleteOne({ _id: doc._id }); // Correct syntax for deleteOne
