import appointment from "../MODLES/appointmentModel.js"; 
...
// Get appointment by ID
 const getAppointmentById = async (req, res) => {
    try {
        const app = await appointment.findOne({ appointment_id: req.params.id }).populate('doctor_id','name specialization').populate("patient_id");
        if (!app) {
...
const getAppointmentsByDoctorId = async (req, res) => {
    try {
        const doc= await doctor.find({doctor_id:req.params.id})
      //  console.log(doc)
      //  console.log(doc[0]._id)
        const appointments = await appointment
            .find({ doctor_id: doc[0]._id }).select("-_id").select("-__v").select("-doctor_id");
...
const getAppointmentsByPatientId = async (req, res) => {
    try {
        const pai= await patient.find({patient_id:req.params.id})
    //  console.log(pai)
    // console.log(pai[0]._id)
        const appointments = await appointment
            .find({ patient_id: pai[0]._id }).select("-_id").select("-__v").select("-patient_id");
...
const deleteAppointment = async (req, res) => {
    try {
        const app = await appointment.findOne({ appointment_id: req.params.id });
        
        if (!app) {
            return res.status(404).json({ message: "appointment not found" });
        }

        await app.deleteOne({ _id: app._id }); // Correct syntax for deleteOne
